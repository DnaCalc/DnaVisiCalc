...
impl Engine {
    ...
    pub fn set_number(&mut self, cell: CellRef, number: f64) -> Result<(), EngineError> {
        ...
        if was_formula {
            ...
            self.fec_host.unregister_formula(&FecFormulaId::Cell(cell));
        }
    ...
    pub fn set_name_number(&mut self, name: &str, number: f64) -> Result<(), EngineError> {
        let key = self.normalize_name(name)?;
        let was_formula = matches!(self.names.get(&key), Some(NameEntry::Formula(_)));
        let old_value = self
            .name_values
            .get(&key)
            .map(|stored| stored.value.clone())
    ...
    fn apply_structural_op(&mut self, op: StructuralOp) -> Result<(), EngineError> {
        ...
        self.refresh_fec_dependency_registrations();
        self.committed_epoch += 1;
        self.maybe_recalculate()
    }
        ...
        for scc in &tree.sccs {
            if !scc.is_cyclic {
                for cell in &scc.cells {
                    let (runtime, value) = self.evaluate_cell_via_f3e(&mut evaluator, *cell);
                ...
                for _iteration in 0..max_iter {
                    ...
                    for cell in &scc.cells {
                        let old_value = new_values
                            ...
                            .cloned()
                            .unwrap_or(Value::Number(0.0));
                        let (runtime, new_value) =
                            self.evaluate_cell_via_f3e(&mut evaluator, *cell);
                        runtime_values.insert(*cell, runtime.clone());
...
mod tests {
    ...
    #[test]
    fn fec_registration_tracks_none_and_ref_only_profiles() {
        let mut engine = Engine::new();
        let a1 = CellRef { col: 1, row: 1 };
        let b1 = CellRef { col: 2, row: 1 };

        engine.set_formula(a1, "SIN(0)").unwrap();
        assert_eq!(
            engine
                .fec_host
                .required_capabilities_for(&FecFormulaId::Cell(a1)),
            &[]
        );

        engine.set_formula(b1, "A1+1").unwrap();
        assert_eq!(
            engine
                .fec_host
                .required_capabilities_for(&FecFormulaId::Cell(b1)),
            &[FecCapabilityTag::ReferenceResolution]
        );
    }

    #[test]
    fn fec_registration_is_rebuilt_after_structural_edit() {
        let mut engine = Engine::new();
        engine.set_number_a1("A1", 1.0).unwrap();
        engine.set_formula_a1("B1", "A1+1").unwrap();

        engine.insert_col(1).unwrap();

        assert_eq!(
            engine
                .fec_host
                .required_capabilities_for(&FecFormulaId::Cell(CellRef { col: 3, row: 1 })),
            &[FecCapabilityTag::ReferenceResolution]
        );
    }
}
