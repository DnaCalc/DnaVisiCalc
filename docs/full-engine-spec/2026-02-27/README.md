# Core Engine Spec Pack

## Version Metadata

- `spec_pack_version`: `2026-02-27`
- `status`: `immutable`
- `source`: copied from canonical docs at freeze time

## Included Documents

Normative core contract docs:
- `SPEC_v0.md`
- `ENGINE_REQUIREMENTS.md`
- `ENGINE_API.md`

Informative companion docs:
- `CONTEXT.md`
- `SPEC_v0_INTEGRATION_APPENDIX.md`
- `ENGINE_REQUIREMENTS_INTEGRATION_APPENDIX.md`
- `ENGINE_API_RUST_APPENDIX.md`

## Canonical Source Mapping (at freeze)

- `SPEC_v0.md` <- `docs/SPEC_v0.md`
- `ENGINE_REQUIREMENTS.md` <- `docs/ENGINE_REQUIREMENTS.md`
- `ENGINE_API.md` <- `docs/ENGINE_API.md`
